#!/usr/bin/env node
//
// Renders a multi-file brochure folder into a single merged PDF.
//
// Usage:
//   node server/render-brochure.js materials/<brochure-name>/
//
// Pages are read from pages.json in the target folder. Falls back to
// PAGES_FALLBACK for legacy brochures without a pages.json.
...
if (!input) {
  console.error("Usage: node server/render-brochure.js <brochure-folder>");
  console.error("  e.g. node server/render-brochure.js materials/my-brochure/");
  process.exit(1);
}
